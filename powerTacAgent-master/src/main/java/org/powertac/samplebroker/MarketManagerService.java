...
implements MarketManager, Initializable, Activatable
{
  ...
  private PortfolioManagerService pms; //custom code
  ...
  //custom code for weather predictions
  ...
  public void initialize (BrokerContext broker)
  {
    ...
    //custome code
    this.pms=new PortfolioManagerService(); 
	pms.initialize(broker);//custom code
  }
  ...
  public synchronized void activate (int timeslotIndex)
  {
    ...
    System.out.println("inside activate"+ iter++);
  ...
  private void submitOrder (double neededKWh, int timeslot)
  {
    ...
    Double l=limitPrice;
	/*
	 * if(limitPrice==null) l=limitPrice; else l= MCTS_fun(limitPrice,
	 * (limitPrice/(Double)10.0),5 ,24);
	 */
    Order order = new Order(broker.getBroker(), timeslot, this.heuristicWeather()*neededMWh, l);
    lastOrder.put(timeslot, order);
    broker.sendMessage(order);
  }
  
  Double MCTS_fun(Double limitPrice, double sigma, int numOfChildren, int hoursAhead) {
	  int i=0;
	  Node root=new Node(null,limitPrice, sigma, numOfChildren, this.pms);
	  MCTS mc=new MCTS(root);
	  while(i<10000) {
		  System.out.println("inside mcts"+ iter++);
		  mc.treeSearch(hoursAhead);
		  i++;
	  }
	  int bestChild= root.maxAverageChild();
	  return limitPrice+(bestChild-Math.floor(numOfChildren/2))*(sigma);
  ...
  //---------Heuristic for weather forcasting-------------------------
  double heuristicWeather() {
	  WeatherForecast list=this.forecast;
	  int halfDay=15;
	  double avgHalfDay=0.0;
	  double avgNight=0.0;
	  for (int i=0; i<halfDay; i++) {
		  avgHalfDay+=list.getPredictions().get(i).getTemperature();
	  }
	  avgHalfDay=avgHalfDay/halfDay;
	  for (int i=halfDay; i<24; i++) {
		  avgNight+=list.getPredictions().get(i).getTemperature();
	  }
	  avgNight=avgNight/(24-halfDay);
	  

	  if((avgHalfDay<12.0  && avgNight<8.0)|| (avgHalfDay>=25.0  && avgNight>22.0))  //either it is too cold or too hot
		  return 1.1;
	  return 1.0; 
