...
implements PortfolioManager, Initializable, Activatable
{
	...
	public Map<PowerType, List<TariffSpecification>> competingTariffs;
	...
	public void initialize (BrokerContext context)
	{
		...
		this.propertiesService=new BrokerPropertiesService(); //custom
	...
	List<TariffSpecification> getCompetingTariffs (PowerType powerType)
	{
		...
		ArrayList<TariffSpecification> resultTmp = new ArrayList<TariffSpecification>();
		if(competingTariffs!=null) {
		...
		//if (competingTariffs==null)
		List<TariffSpecification> result=resultTmp;
		competingTariffs.put(powerType, result);
		return result;
	}
	/**
	 * custom method similiar with the previous
	 * but it returns the average compiting tariff price
	 * independatly from the power type
	 * @return
	 */

	ArrayList<TariffSpecification> getAllCompetingTariffs(){
		ArrayList<TariffSpecification> resultTmp = new ArrayList<TariffSpecification>();
		if(competingTariffs!=null) {

			List<List<TariffSpecification>> result = new ArrayList(competingTariffs.values());
			for(List<TariffSpecification> l: result) {
				for(TariffSpecification spec: l) {
					resultTmp.add(spec);
				}
			}
			return resultTmp;
	...
	public synchronized void handleMessage (TariffSpecification spec)
	{
		...
		System.out.println("New "+spec.toString());
		...
		if (brokerContext.getBrokerUsername().equals(theBroker.getUsername())) {
			...
			System.out.println("Spec was added!");
		}

		System.out.println("new tariff: ");
		System.out.println("the id of the tariff is "+spec.getId());
		System.out.println("and the last rate of that is "+spec.getRates().get(spec.getRates().size()-1).getValue());
	}
	...
	public synchronized void activate (int timeslotIndex)
	{
		if (customerSubscriptions.size() == 0) {
			...
			improveTariffs(PowerType.CONSUMPTION);
		}
	...
	private void improveTariffs(PowerType pt)
	{
      ...
      for (TariffSpecification spec: candidates) {
        if (spec.getBroker() == brokerContext.getBroker()) {
          ...
          brokerContext.sendMessage(ece);
			...
			List<TariffSpecification> candidates =
					tariffRepo.findTariffSpecificationsByPowerType(pt);
			
			for (TariffSpecification spec: candidates) {
				//double periodicPayment=spec.getPeriodicPayment();
				List<Rate> rates=spec.getRates();
				double limitPrice=rates.get(rates.size()-1).getValue();
				double sigma;
				
				sigma=0.1;
				Node bestTactic=MCTS_fun(limitPrice, sigma, 3, 24);
				double bestBid=bestTactic.ithMaxAverageChild(1).limitPrice;
				//if you want to run the multi hours strategy
				...
				brokerContext.sendMessage(spec);
//---------------------------------------------------------------------------------------//
				//code for hourly bids and rates
				/*for(int i=0; i<5; i++) {
					if(bestBid!=Double.MIN_VALUE){
						System.out.println("inside rating");
						Double newLimitPrice=bestTactic.ithMaxAverageChild(i+1).limitPrice;
						if(newLimitPrice==Double.MIN_VALUE) {
							TariffRevoke revoke =
									new TariffRevoke(brokerContext.getBroker(), spec);
							brokerContext.sendMessage(revoke);
							tariffRepo.addSpecification(spec);
						...
						Rate r=new Rate().withValue(newLimitPrice);
						r.withDailyBegin(i)
						 .withDailyEnd(i+1);
						spec.addRate(r);
						tariffRepo.addSpecification(spec);
...
//------------end of hourly ahead plan strategy----------------------------------------
	...
	//custom method an alternative of the above
	/**
	 * 
	 * @return a has map with key the ""+PowerTYpe and value the total subscribers of that powertype
	 */
	HashMap<String, Integer> customGetCustomerCounts(){
		HashMap<String, Integer> result = new HashMap<>();
		for (TariffSpecification spec : customerSubscriptions.keySet()) {
			Map<CustomerInfo, CustomerRecord> customerMap = customerSubscriptions.get(spec);
			//sth like select sum(subscribers) from tariffs groupBy powerType
			for (CustomerRecord record : customerMap.values()) {
				Object a=result.get(""+spec.getPowerType());
				int b;
				if(a==null) b=0;
				else b=result.get(""+spec.getPowerType());
				result.put(""+spec.getPowerType(), 
						b+record.subscribedPopulation);
			}
		}
		return result;
	}
	...
	//Our methods form mcts etc
	Node MCTS_fun(Double limitPrice, double sigma, int numOfChildren, int hoursAhead) {
		int i=0;
		
		Node root=new Node(null,limitPrice, sigma, numOfChildren, this);
		MCTS mc=new MCTS(root);
		
		//System.out.println("pms of root is "+this);
		while(i<14000) {


			mc.nextIteration();
			root=mc.treeSearch(hoursAhead);
			i++;
		}

		//return root;
		//check the maxAverageChildNode();
		/* if(bestChild==0) return Double.MIN_VALUE;
	  return limitPrice+(bestChild-1-numOfChildren*(sigma)); //-1 because i==0 is the revoke child
		 */
		
		/*if(bestChild==null) return Double.MIN_VALUE;// Fix value in order to revoke
		return bestChild.limitPrice; //limit price is the value of the bid in the node bestChild
		*/
		return root;
	}
